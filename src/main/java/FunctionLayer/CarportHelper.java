...
public class CarportHelper {
    ...
    public CarportHelper(int carportLengthCM, int carportWidthCM, int carportHeight, int shedLength, int shedWidth, int carportPitch) throws UniversalSampleException {
        ...
        this.rolesOfBand = calcTest.calcRolesAmountBand(band);
        ...
        this.amountOfTiles = calcTest.calcRoofTileAmount(carportLengthCM, carportWidthCM);
        this.amountOfScrews = calcTest.calcRoofScrewAmount(carportLengthCM, carportWidthCM, BOTTOMSCREW_ID);
        this.amountOfUniversalScrews = calcTest.calcUniversalScrews(carportLengthCM, hasPitch);
        this.amountOfPlankWaterScrews = calcTest.calcPlankAndWaterScrews();
        this.amountOfBracketScrews = calcTest.calcBracketScrews(carportLengthCM, hasPitch);
        this.amountOfCarriageBolts = calcTest.calcCarriageBolts(carportLengthCM, hasShed, shedLength, shedWidth, carportWidthCM);
        this.transomSides = calcTest.calcTransomsLengthSides(shedLength);
        this.transomFrontAndBack = calcTest.calcTransomsLengthFrontAndBack(shedWidth);
        this.headsInShed = calcTest.calcHeadsInShed(shedLength);
        this.planksForShedAmount = calcTest.calcPlanksForShed(shedLength, shedWidth);
        this.packageOfOuterScrews = calcTest.calcOuterScrewsShed(shedLength, shedWidth, OUTERSCREW_ID);
        this.packageOfInnerScrews = calcTest.calcInnerScrewsShed(shedLength, shedWidth, INNERSCREW_ID);
        this.amountOfAngleMount = calcTest.calcAngleMount(shedLength, shedWidth);
        this.tilesForPitchedRoof = calcTest.calcTilesForPitchedRoof();
        this.packagesOfTileBindersAndHooks = calcTest.calcPackagesOfTileBindersAndHooks();
        this.amountOfRooftileStones = calcTest.calcAmountOfRoofTileStones(carportLengthCM);
        this.amountOfRooftileStoneBrackets = calcTest.calcAmountOfRoofTileStoneBrackets(amountOfRooftileStones);
        this.amountOfToplathHolders = calcTest.calcAmountOfToplathHolders(amountOfRafts);
        this.gavlPlankLength = calcTest.calcGavlPlanksLength(carportWidthCM, carportPitch);
        this.amountOfGavlPlank = calcTest.calcAmountOfGavlPlanks();
